...
def request_iors(fid):
    result = []
    try:
        result = STIX_MDB.select_iors_by_id(int(fid))
    except ValueError:
        pass
    return json_util.dumps(result)

@ior_manager.route("/request/ior/info/last")
def request_last_iors_info():
    result = []
    try:
        result = STIX_MDB.select_last_iors_info()
    except ValueError:
        pass
    return json_util.dumps(result)
...
def request_list_of_ior_by_tw():
    ...
    if request.method == 'POST':
        try:
            ...
            if int(time_type) == 2:
                use_gen_time = False
            result = STIX_MDB.select_iors_by_tw(
...
@ior_manager.route("/request/ior/info", methods=['POST'])
def request_list_of_ior_by_filename():
    result = []
    if request.method == 'POST':
        try:
            ...
            result = STIX_MDB.select_iors_by_filename(filename)
        except (TypeError, ValueError, IndexError):
            pass
    return json_util.dumps(result)
