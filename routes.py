...
#@app.route('/view/list/calibration')
#def view_calibration_run_list():
#    runs=STIX_MDB.select_all_calibration_runs()
#    return render_template('list-calibration.html',runs=runs)
...
def view_calibration_packets(calibration_id):
    return render_template('packet-request.html',calibration_id=calibration_id)
...
def view_calibration(calibration_id):
    ...
    return render_template('plot-calibration.html', calibration_id=calibration_id, message=message)
...
def view_file_housekeeping(file_id):
    ...
    return render_template('plot-housekeeping.html', file_id=file_id, message=message)
...
def render_housekeeping(start_unix=0,span_sec=0, message=""):
    return render_template('plot-housekeeping.html', start_unix=start_unix,span_sec=span_sec, message=message)
    ...
    if request.method == 'POST':
        try:
            ...
            if selection_type=='SPID':
                ...
                if val:
                    SPID=int(val)
                    status, packets=STIX_MDB.select_packets_by_SPIDs(SPID, start_unix,span_sec, header_only=True)
...
def load_packets_of_file(file_id, group):
    ...
    status, packets=STIX_MDB.select_packets_by_run(file_id, SPIDs=SPIDs, header_only=False)
    result={'status':status,'packets':packets}
    return json_util.dumps(result)
...
def load_headers_of_calibration(calibration_id):
    status, packets=STIX_MDB.select_packets_by_calibration(calibration_id, header_only=True)
    result={'status':status,'packets':packets}
    return json_util.dumps(result)
...
def request_quicklook_pdf(run_id):
    ...
    if abs_filename:
        ...
        return send_from_directory(directory=path_name,
                       filename=filename,
...
def request_calibration_runs():
    ...
    if start_unix> 0 and span_seconds>0:
        status, data=STIX_MDB.select_calibration_runs_by_tw(start_unix,span_seconds)
